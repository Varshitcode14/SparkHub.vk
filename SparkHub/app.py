...
def respond_to_date_request():
    ...
    if response == 'accept':
        ...
        new_request = DateRequest(
            sender_id=current_user.id,
            receiver_id=date_request.sender_id if date_request.receiver_id == current_user.id else date_request.receiver_id,
...
def api_users():
    ...
    filters = {
        ...
        'state': request.args.get('state'),
        'relationship_looking_for': request.args.get('relationship_looking_for'),
        'branch': request.args.get('branch'),  # Add this line
    }
...
def update_profile():
    try:
        ...
        user.communication_style = request.form.get('communication_style', user.communication_style)
        user.state = request.form.get('state', user.state)
        user.relationship_looking_for = request.form.get('relationship_looking_for', user.relationship_looking_for)
        cgpa = request.form.get('cgpa')
        if cgpa:
...
def confessions():
    ...
    if current_user.is_authenticated:
        likes = ConfessionLike.query.filter_by(user_id=current_user.id).all()
...
def submit_confession():
    ...
    author = None if is_anonymous else current_user.name
