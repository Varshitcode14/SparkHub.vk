document.addEventListener("DOMContentLoaded", () => {
  ...
  if (signupForm) {
    ...
    signupForm.addEventListener("submit", (e) => {
      ...
      if (signupForm.checkValidity()) {
        ...
        formData.set("societies", Array.from(formData.getAll("societies")).join(","))
  ...
  function loadBlindDateProfile(userId) {
    fetch(`/profile/${userId}`)
      .then((response) => response.json())
      .then((data) => {
        const modalBody = document.getElementById("blindDateProfileModalBody")
        modalBody.innerHTML = `
                ...
                `
        const modal = new bootstrap.Modal(document.getElementById("blindDateProfileModal"))
        modal.show()
      })
      .catch((error) => {
  ...
  function attachButtonListeners() {
    // Update the view profile button listener for blind dates
    document.querySelectorAll(".view-blind-profile").forEach((button) => {
      button.addEventListener("click", function () {
        const userId = this.getAttribute("data-user-id")
        loadBlindDateProfile(userId)
      })
    ...
    // Regular view profile buttons (for non-blind dates)
